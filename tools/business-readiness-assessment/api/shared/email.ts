...
export async function sendAssessmentEmail(options: EmailOptions): Promise<void> {
  ...
  if (!logicAppUrl) {
    console.warn('LOGIC_APP_EMAIL_URL not configured - email sending skipped in local development');
    return;
  }
  ...
  try {
    ...
    const attachmentName = `Business-Readiness-Report-${options.companyName.replace(/\s+/g, '-')}.pdf`;

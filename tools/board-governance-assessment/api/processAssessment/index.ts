/**
 * AI Board Governance Diagnostic - Main Azure Function Handler
 * Processes board governance assessment submissions and orchestrates all services
 */
...
export async function processAssessment(
  ...
  context.log('Processing Board Governance assessment submission');
  ...
  try {
    ...
    context.log(`Governance score calculated: ${scoringResult.data.governance_score} (${scoringResult.data.band_name})`);
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development' && pdfBase64) {
        ...
        const filename = `Board-Governance-Report-${submission.company_name.replace(/\s+/g, '-')}-${Date.now()}.pdf`;
    ...
    try {
      await sendAssessmentEmail({
        to: submission.email,
        subject: `Your Board AI Governance Report - ${submission.company_name}`,
    ...
    try {
      ...
      if (notificationEmail) {
        await sendAssessmentEmail({
          ...
          subject: `New Board Governance Assessment: ${submission.company_name} - Score: ${scoringResult.metadata.final_score}`,
          pdfBase64: pdfBase64,
          recipientName: 'Team',
          companyName: submission.company_name,
          score: scoringResult.metadata.final_score,
    ...
    context.res = {
      ...
      body: {
        success: true,
        governance_score: scoringResult.metadata.final_score,
        governance_band: scoringResult.data.band_name,
        message: "Board governance assessment processed successfully. Check your email for the full report."
      }
    };
